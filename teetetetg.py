...
def summarize_with_model(query: str, corpus_lines, corpus_dict, store, bm25_params, source_weights, top_k=15):
...
if __name__ == "__main__":
    ...
    # 加载testllm.json测试数据
    with open('testdata.json', 'r', encoding='utf-8') as f:  # 替换为实际路径
    ...
    for query in queries:
        ...
        answer = summarize_with_model(query, corpus_lines, corpus_dict, store, bm25_params, source_weights)
        print("总结答案：", answer)
        # 计算单个查询耗时
        query_time = time.time() - query_start_time
        total_query_time += query_time

    total_program_time = time.time() - start_time
    print(f"⏱ 总生成处理时间: {round(total_query_time, 2)} 秒")
    print(f"⏱ 程序总运行时间: {round(total_program_time, 2)} 秒")
