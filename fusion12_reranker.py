...
def calculate_entropy_weights(vector_results, keyword_results):
    # 收集所有唯一文档内容
    all_contents = set()
    ...
    if n == 0:
        # 默认权重
        return {'vectorsearch': 0.5, 'keywordsearch': 0.5}

    score_matrix = np.zeros((n, 2))  # 列: [vectorsearch, keywordsearch]
...
def query_and_fuse(query, corpus_lines, corpus_dict, store, bm25_params, reranker=None):
    ...
    return vector_results, keyword_results, fused_results, source_weights
...
if __name__ == "__main__":
    ...
    reranker_path = r"D:/Project_code/SystemonRSDprocessingKG/bge-reranker-base"
    ...
    for query in queries:
        # 修改调用参数，移除固定权重
        vector_results, keyword_results, fused_results, source_weights = query_and_fuse(
